...
//   Outputs stream out via AXI4-Stream after PIPE_DEPTH=8 clock latency.
//
...
//   S2 : Online max update - running max for numerically stable softmax
//   S3 : Exp LUT lookup    - 8-entry exp approximation per byte
//   S4 : Running sum       - softmax denominator accumulation
//   S5 : Softmax normalize - scaled division by running sum
//   S6 : Welford mean      - online mean update for layer norm
//   S7 : Welford variance  - online M2 update for layer norm
//   S8 : LayerNorm output  - scale and shift, write to output
//
// Port list:
//   clk            I  1    System clock (single domain)
//   rst_n          I  1    Async active-low reset
//   s_axis_tdata   I  64   Input beat (8 x INT8 elements)
//   s_axis_tvalid  I  1    Input valid
//   s_axis_tlast   I  1    Last beat of row
//   s_axis_tready  O  1    Core ready (back-pressure)
...
//   precision      I  1    0=INT8 (implemented), 1=FP64
//   start          I  1    Start signal from control FSM
//   done           O  1    Done pulse: high when tlast exits pipeline
// =============================================================================
...
module compute_core #(
    ...
    // Exp LUT: exp(-k/8)*256 for k=0..7
    // Values: exp(0)=1.000->255, exp(-1/8)=0.882->225... approximated
    reg [7:0] exp_lut [0:7];

    integer b;

    // -------------------------------------------------------------------------
    // LUT initialization (ROM — constant array)
    // -------------------------------------------------------------------------
    initial begin
        exp_lut[0] = 8'd255;
        exp_lut[1] = 8'd224;
    ...
    // -------------------------------------------------------------------------
    // S1 — Input latch + back-pressure
    // -------------------------------------------------------------------------
    always @(*) begin
        s_axis_tready = m_axis_tready | ~pipe_valid[0];
    end
    ...
    // -------------------------------------------------------------------------
    // S2 — Online max (first byte representative)
    // -------------------------------------------------------------------------
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            pipe_data[1]  <= 64'd0;
    ...
    // -------------------------------------------------------------------------
    // S3 — Exp LUT lookup
    // FIX: extract each byte explicitly to avoid variable part-select
    // in always_comb which Icarus 12 does not support.
    // -------------------------------------------------------------------------
    reg [7:0] byte0, byte1, byte2, byte3, byte4, byte5, byte6, byte7;
    reg [AXIS_W-1:0] exp_beat;

    always @(*) begin
        byte0 = pipe_data[1][ 7: 0];
        byte1 = pipe_data[1][15: 8];
        byte2 = pipe_data[1][23:16];
        byte3 = pipe_data[1][31:24];
        byte4 = pipe_data[1][39:32];
        byte5 = pipe_data[1][47:40];
        byte6 = pipe_data[1][55:48];
        byte7 = pipe_data[1][63:56];
    ...
    // -------------------------------------------------------------------------
    // S5 — Softmax normalize: out = (exp * 255) / running_sum
    // FIX: unrolled loop, no variable part-select
    // -------------------------------------------------------------------------
    ...
    always @(*) begin
        eb0 = pipe_data[3][ 7: 0];
        eb1 = pipe_data[3][15: 8];
        eb2 = pipe_data[3][23:16];
        eb3 = pipe_data[3][31:24];
        eb4 = pipe_data[3][39:32];
        eb5 = pipe_data[3][47:40];
        eb6 = pipe_data[3][55:48];
        eb7 = pipe_data[3][63:56];
        ...
        if (!rst_n) begin
            ...
            if (pipe_valid[5]) begin
                welford_m2 <= pipe_last[5] ? 24'd0 :
                    welford_m2 + ($signed({16'd0, pipe_data[5][7:0]}) -
                                  $signed(welford_mean)) *
                                 ($signed({16'd0, pipe_data[5][7:0]}) -
                                  $signed(welford_mean));
            end
        end
    end
    ...
    // -------------------------------------------------------------------------
    // S8 — LayerNorm output (g=1, b=0 stub; full arithmetic in M3)
    // -------------------------------------------------------------------------
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            pipe_data[7]  <= 64'd0;
